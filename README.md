...
2. 发现有没被 STATE 记录的改动（通常只是额度用完前最后一小段），把改动简要列给你，问一句当时想做什么，再补进 STATE；
...
- [!] 3. 翻译 —— 受阻：还没有翻译接口的 API Key，需要你提供
## 有效决定
...
这是备用手段，不是每次都用。STATE + git 已经能恢复绝大部分进度，漏记的那一小段问你一句最准。只有老项目第一次接入（从没建过 STATE）、或者你也记不清当时为什么这么做时，Agent 才会去读上个 Agent 的聊天记录。
